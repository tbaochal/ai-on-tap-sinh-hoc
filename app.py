...
def doc_pool_hs_theo_pham_vi(khoi="", chuong="", bai=""):
    ...
    - Ngân hàng tốt nghiệp là kho riêng và không tham gia Ôn bài/Ôn chương.
    """
    khoi_chuan = _chuan_khoi_hs_fast(khoi)
...
def tao_de_giao_vien():
    ...
    with tab2:
        ...
        st.caption(
            ...
            "thời gian 50 phút. Mã đề chỉ rút từ **Ngân hàng tốt nghiệp** và giữ nguyên câu, ảnh/bảng của câu gốc."
        )
        ...
        if not bank12:
            ...
            st.info(
                "Mỗi mã đề mới được rút **100% từ Ngân hàng tốt nghiệp**, trộn nhiều file nguồn và không lặp câu trong cùng mã đề."
            )
...
def hien_thi_du_lieu_truc_quan_cau(q, show_source=True):
    data = q.get("du_lieu_truc_quan", {}) or {}
    if not data:
        return
...
def luu_ngan_hang_tot_nghiep_thuc_te(ds):
    ...
    try:
        for key in list(st.session_state.keys()):
            if (
                str(key).startswith("_hs_pool_scope_")
                or str(key).startswith("_hs_grad_common_candidates_")
            ):
                st.session_state.pop(key, None)
    except Exception:
        pass
...
def _grad_parse_question_structure(part, qnum, lines, resources, answer_map, source_name):
    ...
    # Phân loại metadata phục vụ quản trị/phân tích Ngân hàng tốt nghiệp.
    # Kho tốt nghiệp vẫn tách riêng và không được nhập vào Ngân hàng chung.
    try:
    ...
    for child in doc.element.body.iterchildren():
        ...
        if tag.endswith("}p"):
            ...
            if current is not None and not in_answer_section and rows:
                ...
                # Ảnh trong ô bảng không xuất hiện như paragraph cấp body.
                # Quét thêm từng paragraph của từng cell để giữ đủ hình của đề nguồn.
                for row in tbl.rows:
                    for cell in row.cells:
                        for para_cell in cell.paragraphs:
                            image_resources, image_counter = _grad_extract_images_from_paragraph(
                                para_cell, doc, media_dir, safe_stem, image_counter
                            )
                            for res in image_resources:
                                res2 = dict(res)
                                res2["nguon"] = source_name
...
def hien_thi_tai_nguyen_cau_tot_nghiep(q, show_source=True):
    resources = q.get("tai_nguyen_truc_quan", []) or []
    if not resources:
        # Tương thích dữ liệu cũ.
        data = q.get("du_lieu_truc_quan", {}) or {}
        if data:
            hien_thi_du_lieu_truc_quan_cau(q, show_source=show_source)
        return

    da_hien_thi = False
    for res in resources:
        loai = _grad_norm_text(res.get("loai", ""))
        co_dau_hieu_anh = any(
            str(res.get(k, "") or "").strip()
            for k in ("duong_dan", "duong_dan_anh", "url_anh", "public_url", "du_lieu_base64", "image_base64", "base64")
        )
        if loai in {"anh", "hinh", "hinh anh", "image", "hinh tu tai lieu"} or co_dau_hieu_anh:
            if _hien_thi_anh_tu_resource(res, width=GRAD_IMAGE_DISPLAY_WIDTH):
                da_hien_thi = True
        elif loai in {"bang", "bang so lieu", "table"}:
            rows = res.get("du_lieu") or []
            if rows:
                width = max(len(r) for r in rows)
    ...
    # Dữ liệu cũ có thể chỉ lưu ở trường tương thích. Nếu list tài nguyên
    # không hiển thị được, thử nhánh này để không mất hình/bảng ở giao diện HS.
    if not da_hien_thi and (q.get("du_lieu_truc_quan") or {}):
        hien_thi_du_lieu_truc_quan_cau(q, show_source=show_source)
...
def rut_de_tot_nghiep_tu_de_that(bank, seed=None):
    """
    Rút form 18/4/6 CHỈ từ Ngân hàng tốt nghiệp.

    - Không đọc/ghép Ngân hàng chung.
    - Không trùng nội dung trong cùng mã đề.
    - Ưu tiên trộn câu từ nhiều file nguồn.
    - Giữ nguyên tài nguyên trực quan (ảnh/bảng) đi kèm câu.
    """
    ...
    for dang, n in specs.items():
        ...
        picked = _grad_round_robin_sample(pool_grad, n, rng)
        picked_final = []
        for q in picked:
            fp = _grad_fingerprint_noi_dung_chung(q)
            if fp in used_fp:
                continue
            picked_final.append(q)
            used_fp.add(fp)
            if len(picked_final) >= n:
                break

        selected.extend(picked_final)
        if len(picked_final) < n:
            missing.append({
                "Dạng": dang,
                "Cần": n,
                "Có thể rút": len(picked_final),
                "Thiếu": n - len(picked_final),
            })
...
def xay_dung_ngan_hang_tot_nghiep():
    ...
    # ------------------------------------------------------
    # ẢNH CŨ: NHÚNG BỀN VỮNG ĐỂ LINK HỌC SINH HIỂN THỊ
    # ------------------------------------------------------
    with st.expander("🖼️ Ảnh câu hỏi trên link học sinh", expanded=False):
        st.caption(
            "Nếu một số câu cũ có hình khi chạy VS Code nhưng link học sinh không thấy, "
            "hãy bấm nút dưới đây **trên máy đã nhập các đề Word**. Hệ thống sẽ nhúng ảnh "
            "đang còn trên máy vào dữ liệu dùng chung; không thay đổi nội dung câu hỏi."
        )
        if st.button(
            "🖼️ CỦNG CỐ ẢNH CŨ CHO LINK HỌC SINH",
            use_container_width=True,
            key="grad_persist_old_images",
        ):
            bank_img = doc_ngan_hang_tot_nghiep_thuc_te()
            so_cau_co_anh = sum(
                1 for q in (bank_img or [])
                if any(
                    str(res.get("du_lieu_base64", "") or "").strip()
                    for res in (q.get("tai_nguyen_truc_quan", []) or [])
                    if isinstance(res, dict)
                )
                or str((q.get("du_lieu_truc_quan", {}) or {}).get("du_lieu_base64", "") or "").strip()
            )
            if not bank_img:
                st.warning("Ngân hàng tốt nghiệp hiện chưa có câu để xử lý.")
            elif luu_ngan_hang_tot_nghiep_thuc_te(bank_img):
                st.success(
                    f"Đã lưu lại ngân hàng; hiện có **{so_cau_co_anh} câu** mang ảnh nhúng bền vững để link học sinh đọc."
                )
            else:
                st.error("Chưa lưu được dữ liệu ảnh. Hãy kiểm tra kết nối rồi thử lại.")
...
def hoc_sinh():
    ...
    st.markdown(
        ...
        <div class="hs-login-shell">
            <div class="hs-login-banner">
                ...
                <div class="hs-login-note">Nhập mã học sinh rồi bấm Đăng nhập để vào khu vực học tập.</div>
            </div>
        </div>
        """,
        unsafe_allow_html=True
    ...
    if not st.session_state.get("hs_portal_open", False):
        ...
        st.caption("Đăng nhập thành công. Bấm **VÀO HỌC** để bắt đầu.")
        return
    ...
    if not st.session_state.hs_dang_lam:
        ...
        if mode_now_fast not in mode_options_fast:
            ...
            st.caption("Chọn nội dung em muốn học hoặc luyện tập.")
    ...
    bank = []
    hs_goi_y_hom_nay = []
    # Luyện tốt nghiệp chỉ tải Ngân hàng tốt nghiệp khi HS bấm BẮT ĐẦU.
    ...
    if not st.session_state.hs_dang_lam:
        ...
        if can_chi_muc:
            ...
            # Chỉ đọc kho tốt nghiệp khi HS thực sự bắt đầu lượt luyện.
            defer_grad_load = (
                _chuan_khoi_hs_fast(hs_khoi_query) == "Khối 12"
            )
            bank = []

        # Lịch sử không cần tải ngân hàng. Ôn bài/chương chưa cần tải lịch sử.
        if (
            che_do not in {"📈 Lịch sử & tiến bộ"}
            and not bank
            and (can_chi_muc or can_bank_day_du or can_tot_nghiep)
            and not (can_tot_nghiep and defer_grad_load)
        ):
            if can_tot_nghiep:
    ...
    if not st.session_state.hs_dang_lam:
        ...
        if che_do == "🎯 Luyện theo gợi ý hôm nay":
            if not hs_goi_y_hom_nay:
                ...
                st.info(
                    "🎯 Lượt luyện sẽ ưu tiên nhiều hơn cho những nội dung em còn yếu. "
                    "Sau mỗi lượt, mức ưu tiên sẽ được cập nhật theo kết quả của em."
                )
                ...
                if pool:
                    ...
                    if len(pool) < 10:
                        st.caption(
                            f"Hiện có {len(pool)} câu phù hợp để luyện theo kết quả gần đây của em."
                        )
                else:
                    so_cau = 0
                ...
                st.success(
                    "Lượt luyện ưu tiên **kiến thức em đang yếu**, câu chưa gặp và câu em từng làm sai."
                )
        ...
        # --------------------------------------------------
            ...
            if defer_scope_load:
                ...
                st.info("Chọn số câu rồi bấm **BẮT ĐẦU LƯỢT LUYỆN**.")
                ...
                if not ds_dot_hien:
                    ...
                    if luot_da_nop:
                        ...
                        st.info(
                            ...
                            "vẫn cùng nội dung, mức độ, dạng câu và cơ cấu điểm."
                        )
                        st.caption(
        ...
        # --------------------------------------------------
            ...
            # Không tải câu hỏi ở bước mở màn hình. Chỉ cấu hình form 18/4/6.
            dang_counts = {
                "Trắc nghiệm 4 lựa chọn": 18,
                "Đúng / Sai": 4,
                "Trả lời ngắn": 6
            ...
            st.info(
                "Mỗi lượt gồm **28 câu**: **18 câu 4 lựa chọn + 4 câu Đúng/Sai + 6 câu Trả lời ngắn**. "
        ...
        if pool or defer_scope_load or defer_grad_load:
            ...
            if st.button(
                ...
                # TỐT NGHIỆP: chỉ đọc Ngân hàng tốt nghiệp.
                if defer_grad_load:
                    with st.spinner("Đang chuẩn bị mã đề tốt nghiệp..."):
                        bank_tot_nghiep_hs_fast = doc_ngan_hang_tot_nghiep_thuc_te()
                        pool12 = [
                            q for q in (bank_tot_nghiep_hs_fast or [])
                            if cau_tot_nghiep_du_dieu_kien_su_dung(q)
                        ]

                    if not pool12:
                        st.warning(
                            "Hiện chưa đủ câu phù hợp để tạo đề. Vui lòng thử lại sau hoặc báo giáo viên."
                        )
                        return
                    pool = pool12
                ...
                if che_do in {CHE_DO_DE_GV, CHE_DO_KIEM_TRA_MA_TRAN}:
                    ...
                    if thieu_luot:
                        st.error("Hiện chưa đủ câu để tạo trọn một đề 18/4/6. Vui lòng báo giáo viên.")
                        st.stop()
    ...
    if (
        ...
        for i, q in enumerate(
            ...
            if q.get("tai_nguyen_truc_quan") or q.get("du_lieu_truc_quan"):
                # HS chỉ cần thấy chính hình/bảng phục vụ câu hỏi; không hiện metadata nguồn.
                hien_thi_tai_nguyen_cau_tot_nghiep(q, show_source=False)
    ...
    if (
        ...
        st.caption(
            "Tiến bộ được tổng hợp qua nhiều câu và nhiều lượt luyện, không chỉ dựa vào một câu đúng."
        )

        st.caption("Kết quả được chấm theo đáp án chuẩn của bài.")
        ...
        if che_do_hien_tai == CHE_DO_DE_GV:
            st.info(
                ...
                "Phần trắc nghiệm được chấm tự động trên **7 điểm**; 3 điểm tự luận do giáo viên chấm riêng."
            )
