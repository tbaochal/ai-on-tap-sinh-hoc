...
# NGÂN HÀNG CÂU HỎI
# ==========================================================
def _la_ban_sao_cau_tot_nghiep_trong_ngan_hang_chung(q):
    """Nhận diện bản sao câu đề thật từng bị đưa nhầm vào kho chung ở bản cũ."""
    ...
    # Chỉ coi là câu tốt nghiệp riêng khi có đồng thời dấu vết file/phần/câu gốc.
    # Không loại các câu AI thông thường chỉ tình cờ có nhãn mục đích "tot_nghiep".
    if str(q.get("muc_dich_su_dung", "") or "").strip() == "tot_nghiep":
        if any(
...
def _doc_ngan_hang_processed_cached():
    ...
    # Tách dứt khoát Ngân hàng tốt nghiệp khỏi Ngân hàng câu hỏi chung.
    # Bản sao cũ (nếu từng bị đưa nhầm vào kho chung) chỉ bị ẨN khỏi kho chung,
    # không xóa dữ liệu gốc; câu thật vẫn nằm trong GRAD_REAL_BANK_PATH.
    data = [
        q for q in data
        if not _la_ban_sao_cau_tot_nghiep_trong_ngan_hang_chung(q)
    ]
...
def _gv_lay_danh_sach_lop_nhe():
    ...
    if client is not None:
        try:
            ...
            while True:
                ...
                for row in rows:
                    lop = str((row or {}).get("class_name", "") or "").strip()
                    if lop:
                        cac_lop.add(lop)
                if len(rows) < 1000:
                    break
    ...
    if not cac_lop:
        try:
            ds = _doc_json_local(STUDENT_PATH, [])
            for hs in ds if isinstance(ds, list) else []:
                lop = str((hs or {}).get("lop", "") or "").strip()
                if lop:
                    cac_lop.add(lop)
        except Exception:
            pass
    ...
    if not cac_lop:
        try:
            ds = _doc_json_local(HS_HISTORY_PATH, [])
            for lan in ds if isinstance(ds, list) else []:
                pham_vi = (lan or {}).get("pham_vi", {}) or {}
                lop = str(
                    (lan or {}).get("lop")
...
def luu_cau_vao_ngan_hang(question):
    ...
    # Câu tách từ đề thật/đề thi thử thuộc Ngân hàng tốt nghiệp riêng.
    # Không cho bất kỳ luồng cũ nào ghi nhầm loại câu này vào kho chung.
    ...
    # Kho này chỉ dành cho câu ôn tập/kiểm tra chung.
    cau_moi.setdefault(
        "duoc_dung_luyen_hs",
        True
    )
...
def hien_thi_dau_trang_tram_sinh_hoc(khu_vuc="hocsinh"):
    ...
    phu_de = "KHU VỰC GIÁO VIÊN" if la_gv else "HỌC TẬP • ÔN LUYỆN • CHINH PHỤC"
    slogan = (
    ...
    st.markdown(
        f"""
        <style>
        .tram-bio-shell {{ max-width:1120px; margin:0 auto 1.05rem auto; }}
        .tram-bio-head {{
            position:relative; overflow:hidden; border-radius:28px;
            padding:1.45rem 1.4rem 1.35rem 1.4rem;
            background:linear-gradient(118deg,#0b3b78 0%,#087f8c 54%,#16865d 100%);
            box-shadow:0 18px 42px rgba(13,70,100,.15); color:#fff;
            border:1px solid rgba(255,255,255,.16);
        }}
        .tram-bio-head::before {{
            content:""; position:absolute; width:250px; height:250px; border-radius:50%;
            right:-105px; top:-135px; background:rgba(255,255,255,.07);
        }}
        .tram-bio-head::after {{
            content:"🧬"; position:absolute; right:5%; top:50%;
            transform:translateY(-50%) rotate(-10deg); font-size:5.5rem;
            opacity:.10; user-select:none;
        }}
        .tram-bio-center {{
        ...
        .tram-bio-dept {{ font-size:.79rem; font-weight:780; letter-spacing:.055em; opacity:.93; text-transform:uppercase; text-align:center; }}
        .tram-bio-school {{ font-size:1.02rem; font-weight:900; margin-top:.14rem; letter-spacing:.025em; text-align:center; }}
        .tram-bio-title {{ font-size:2.55rem; font-weight:950; line-height:1.03; margin:.70rem 0 .28rem 0; text-align:center; }}
        .tram-bio-sub {{
            display:inline-flex; align-items:center; justify-content:center;
            padding:.30rem .76rem; border-radius:999px;
            background:rgba(255,255,255,.11); border:1px solid rgba(255,255,255,.18);
            font-size:.82rem; font-weight:850; letter-spacing:.035em; opacity:.97;
            text-align:center;
        }}
        .tram-bio-slogan {{ margin-top:.72rem; font-size:1rem; font-weight:720; opacity:.98; text-align:center; }}
        .tram-bio-icons {{
            margin-top:.78rem; display:flex; justify-content:center; align-items:center;
            gap:.48rem; flex-wrap:wrap; width:100%;
        }}
        .tram-bio-chip {{
            padding:.33rem .68rem; border-radius:999px; background:rgba(255,255,255,.10);
            border:1px solid rgba(255,255,255,.17); font-size:.78rem; font-weight:750;
        }}
        @media (max-width:700px) {{
            .tram-bio-head {{ padding:1.25rem .95rem 1.15rem .95rem; border-radius:22px; }}
            .tram-bio-title {{ font-size:2rem; }}
            .tram-bio-dept {{ font-size:.68rem; }}
            .tram-bio-school {{ font-size:.88rem; }}
            .tram-bio-head::after {{ font-size:4rem; right:2%; opacity:.06; }}
        }}
        </style>
        <div class="tram-bio-shell">
            <div class="tram-bio-head">
                <div class="tram-bio-center">
...
def tao_cau_hoi_ai():
    ...
    che_do_xay = st.radio(
        ...
        index=None,
        horizontal=True,
        key="gv_build_bank_mode"
    )

    if not che_do_xay:
        return

    if che_do_xay == "⚡ Xây dựng tự động":
        ...
        tong_hat_giong = _document_count_cached(SEED_BANK_PATH)

        if tong_hat_giong > 0:
            st.success(
                f"🌱 Có **{tong_hat_giong} câu hạt giống** trong kho. "
                "Khi GV bấm xây dựng, hệ thống mới đọc các hạt giống phù hợp với cấu hình hiện tại."
            )
        else:
            st.info(
...
def ket_qua_hoc_sinh():
    ...
    # Chỉ đọc lịch sử của đúng học sinh được GV chọn.
    # Không dùng biến ds toàn cục/không tồn tại.
...
def phan_tich_lop_hoc():
    ...
    ds_lop = _gv_lay_danh_sach_lop_nhe()
    if not ds_lop:
        st.info(
...
def du_lieu_va_tien_bo_hoc_sinh():
    ...
    st.caption(
        "Theo dõi sự tiến bộ từng học sinh và xuất báo cáo từng học sinh theo lớp. "
    ...
    with tab1:
        labels = [
            f"{str(hs.get('ma_hoc_sinh', '')).strip().upper()} – "
            f"{str(hs.get('ho_ten', '')).strip()}"
            for hs in ds_hs_lop
        ]
    ...
    with tab2:
        ...
        export_key = f"_gv_progress_student_export_{chuan_hoa_ten_lop(lop)}"
        file_hs = st.session_state.get(export_key)

        if not isinstance(file_hs, (bytes, bytearray)):
            if st.button(
                "⚙️ CHUẨN BỊ FILE TỔNG HỢP TỪNG HỌC SINH",
                type="primary",
                use_container_width=True,
                key=f"gv_prepare_student_progress_{chuan_hoa_ten_lop(lop)}"
            ):
                with st.spinner("Đang tạo báo cáo Excel..."):
                    st.session_state[export_key] = tao_excel_tong_hop_hoc_sinh_lop(lop)
                st.rerun()
        else:
            st.download_button(
            ...
            if st.button(
                ...
                key=f"gv_refresh_student_progress_{chuan_hoa_ten_lop(lop)}"
            ):
...
def ngan_hang_hat_giong():
    ...
    if st.button("📥 TÁCH & NHẬP HẠT GIỐNG", type="primary", use_container_width=True, key="seed_import_btn"):
        if not files:
            ...
            fp_old = set(seed_by_fp)

            them = trung_seed = trung_main = 0
            # Chỉ các câu thuộc file vừa nhập/cập nhật mới được đồng bộ ở lượt này.
            # Không quét lại toàn bộ kho cũ để tránh số "đồng bộ mới" tăng bất thường.
            seed_ids_can_sync = set()
            bao_cao = []
            tong_file_nhap = max(1, len(files))
            for idx_file_nhap, f in enumerate(files, start=1):
                status_box.info(f"2/4 Đang tách file {idx_file_nhap}/{tong_file_nhap}: {f.name}")
                progress.progress(min(50, 10 + int(40 * idx_file_nhap / tong_file_nhap)))
                ...
                for q in ds_tach:
                    ...
                    if old_exact is not None:
                        ...
                        old_sid = str(old_exact.get("id", "") or "").strip()
                        if old_sid:
                            seed_ids_can_sync.add(old_sid)
                        trung_seed += 1; trung_seed_file += 1
                        continue

                    # Nhập nhanh: không chạy so gần trùng (SequenceMatcher) cho từng câu.
                    # Chống trùng chính xác bằng fingerprint ở trên vẫn giữ nguyên.
                    # Rà soát gần trùng có thể chạy ở bước bảo trì riêng khi cần.
                    ...
                    if fp:
                        seed_by_fp[fp] = q
                    q_sid = str(q.get("id", "") or "").strip()
                    if q_sid:
                        seed_ids_can_sync.add(q_sid)
                    if fp: fp_old.add(fp)
                    them += 1; them_file += 1
            ...
            status_box.info("3/4 Đang đồng bộ câu mới vào Ngân hàng chung...")
            ...
            if ok_seed_save and ok_seed_cloud and ok_bank_cloud:
                st.success(
                    ...
                    f"📚 Đã đồng bộ thêm {da_chuyen} câu mới của chính lượt nhập này vào ngân hàng chính. Không gọi Gemini."
                )
            else:
                st.error(
                    "Dữ liệu đã ghi vào bản local nhưng chưa xác minh ghi thành công lên Supabase. "
                    "App sẽ không lấy bản cloud cũ đè lên dữ liệu local. Hãy kiểm tra kết nối/Supabase rồi thử lại."
                )
...
# ==========================================================
def _grad_nhung_base64_anh_cho_cau(q):
    """Nhúng ảnh tốt nghiệp vào chính dữ liệu câu để link HS luôn đọc được.

    Không phụ thuộc đường dẫn tuyệt đối của máy/link GV. Với dữ liệu cũ, nếu
    ảnh đã nằm trong thư mục du_lieu_truc_quan_tot_nghiep của bản deploy hiện
    tại thì tự bổ sung base64 một lần rồi lưu lại ngân hàng tốt nghiệp.
    """
    q2 = dict(q or {})
    ...
    for res in resources:
        if str(res.get("loai", "")).strip() != "anh":
            continue
        if str(
            res.get("du_lieu_base64")
            or res.get("image_base64")
            or res.get("base64")
            or ""
        ).strip():
            continue

        raw_path = str(res.get("duong_dan", "") or res.get("duong_dan_anh", "") or "").strip()
        candidates = []
        if raw_path:
            candidates.append(raw_path)

            # Đường dẫn có thể được lưu từ máy/link GV khác. Lấy phần tương đối
            # sau thư mục media để thử lại ở deployment hiện tại.
            norm = raw_path.replace("\\", "/")
            marker_media = "du_lieu_truc_quan_tot_nghiep/"
            if marker_media in norm:
                rel = norm.split(marker_media, 1)[1].strip("/")
                if rel:
                    candidates.append(os.path.join(GRAD_MEDIA_DIR, *rel.split("/")))

        img_path = next((p for p in candidates if p and os.path.isfile(p)), "")
        if not img_path:
            continue
    ...
    if resources != (q2.get("tai_nguyen_truc_quan", []) or []):
        q2["tai_nguyen_truc_quan"] = resources

    # Giữ trường tương thích renderer cũ cũng có ảnh nhúng.
    ...
    if data and not str(
        ...
        for res in resources:
            b64 = str(res.get("du_lieu_base64", "") or "").strip()
            if str(res.get("loai", "")).strip() == "anh" and b64:
                data["du_lieu_base64"] = b64
                q2["du_lieu_truc_quan"] = data
                changed = True
                break
...
def doc_ngan_hang_tot_nghiep_thuc_te():
    ...
    for q in ds:
        ...
        q2, changed_img = _grad_nhung_base64_anh_cho_cau(q2)
        if q2 != q or changed_img:
            changed = True
        upgraded.append(q2)
        ...
        if tag.endswith("}p"):
            ...
            if current is not None and img_paths and not in_answer_section:
                for path in img_paths:
                    ...
                    # Ảnh phải đi cùng dữ liệu câu, không phụ thuộc filesystem
                    # riêng của link GV/link HS.
                    try:
                        with open(path, "rb") as f:
                            blob = f.read()
                        if blob:
                            res_anh["du_lieu_base64"] = base64.b64encode(blob).decode("ascii")
                    except Exception:
                        pass
                    current["resources"].append(res_anh)
...
def rut_de_tot_nghiep_tu_de_that(
    ...
    """
    Rút cứng form 18/4/6 CHỈ từ Ngân hàng tốt nghiệp.
    bank_on_tap/toi_da_on_tap được giữ trong chữ ký để tương thích code cũ,
    nhưng tuyệt đối không dùng Ngân hàng ôn tập để tạo đề tốt nghiệp.
    """
    ...
    for dang, n in specs.items():
        ...
        picked_grad = _grad_round_robin_sample(pool_grad, n, rng)
        picked_all = []
...
def xay_dung_ngan_hang_tot_nghiep():
    ...
    noi_dung_grad = st.radio(
        "Nội dung",
        [
            "① Nhập đề vào ngân hàng",
            "② Kiểm tra & duyệt ngân hàng"
        ],
        index=None,
        horizontal=True,
        key="grad_bank_mode"
    ...
    if not noi_dung_grad:
        return

    if noi_dung_grad == "① Nhập đề vào ngân hàng":
...
def quan_ly_diem_on_kiem_tra():
    ...
    export_score_key = (
        f"_gv_score_export_{chuan_hoa_ten_lop(lop)}_"
        f"{hashlib.md5(str([(x.get('id',''), x.get('diem',0)) for x in ds_hien]).encode()).hexdigest()[:10]}"
    )
    file_score = st.session_state.get(export_score_key)

    if not isinstance(file_score, (bytes, bytearray)):
        if st.button(
            "⬇️ XUẤT BẢNG ĐIỂM EXCEL THEO LỚP",
            use_container_width=True,
            key="gv_score_prepare_export"
        ):
            with st.spinner("Đang tạo file Excel..."):
                st.session_state[export_score_key] = tao_excel_bang_diem_lop(ds_hien, lop)
            st.rerun()
    else:
        st.download_button(
            "⬇️ TẢI BẢNG ĐIỂM EXCEL THEO LỚP",
            data=file_score,
...
def tao_pool_luyen_goi_y_hom_nay(profile, bank, cac_muc_tieu):
    ...
    Ngân hàng tốt nghiệp được tách riêng, nên pool này chỉ dùng câu của kho chung.
    """
...
def hoc_sinh():
    ...
    if not st.session_state.hs_dang_lam:
        ...
        if can_chi_muc:
            ...
            # Luyện tốt nghiệp chỉ đọc Ngân hàng tốt nghiệp, không chạm Ngân hàng chung.
    ...
    if not st.session_state.hs_dang_lam:
        ...
        # --------------------------------------------------
            ...
            st.info(
                "Mỗi lượt là một mã đề mới **18/4/6**, lấy **100% từ Ngân hàng tốt nghiệp**. "
                "Không lặp câu trong cùng mã đề và giữ nguyên ảnh/bảng của câu gốc."
            )
    ...
    if (
        ...
        # Chỉ đếm SỐ CÂU CÒN LẠI, ngay dưới câu cuối cùng.
        # Không dùng str(None) vì "None" là chuỗi có nội dung và sẽ đếm sai.
        con_trong = 0
        for idx_q, q_check in enumerate(de_thi, start=1):
            dang_check = q_check.get("dang_cau", "")

            if dang_check == "Đúng / Sai":
                # Một câu Đúng/Sai chỉ hoàn thành khi đủ cả 4 ý a–d.
                da_day_du = all(
                    st.session_state.get(f"hs_answer_{idx_q}_{j}", None) is not None
                    and bool(str(st.session_state.get(f"hs_answer_{idx_q}_{j}", "")).strip())
                    for j in range(1, 5)
                )
            elif dang_check == "Trả lời ngắn":
                value = st.session_state.get(f"hs_short_answer_{idx_q}", None)
                da_day_du = value is not None and bool(str(value).strip())
            else:
                value = st.session_state.get(f"hs_answer_{idx_q}", None)
                da_day_du = value is not None and bool(str(value).strip())
        ...
        if con_trong:
            ...
            st.success(
                "✅ Em đã trả lời đầy đủ tất cả các câu."
            )
